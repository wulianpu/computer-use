...
def _check_against_official_schema(
    root: Path, doc_rel: Path, schema_rel: Path, label: str, checks: list[Check]
) -> dict | None:
    ...
    try:
        ...
        _add(
            checks,
            f"{label} conforms to official Agent Plugins schema",
            not errors,
            "; ".join(errors[:3]) if errors else "pinned 1.0.0 schema",
        )
        ...
        _add(
            ...
            f"{label} conforms to official Agent Plugins schema",
            False,
            f"cannot load pinned schema: {exc}",
        )
    return doc
...
def _check_plugin_policy(manifest: dict | None, checks: list[Check]) -> None:
    ...
    _add(
        checks,
        "policy: no forbidden manifest keys",
        not forbidden,
        f"forbidden: {forbidden}" if forbidden else "",
    )
    _add(
        checks,
        "policy: repository points at a real GitHub origin",
...
def _check_mcp_policy(doc: dict | None, checks: list[Check]) -> None:
    ...
    _add(
        checks,
        "policy: mcp.json == deterministic generator template",
    ...
    if not isinstance(servers, dict) or set(servers) != {SERVER_NAME}:
        _add(
            checks,
            "policy: mcp.json declares exactly server 'cua-driver'",
            False,
    ...
    _add(
        checks,
        "policy: command directly invokes cua-driver",
        command == "cua-driver",
        repr(command),
    )
    _add(checks, "policy: args == ['mcp']", entry.get("args") == ["mcp"], repr(entry.get("args")))
    wrapper_match = FORBIDDEN_COMMAND_PATTERN.search(str(command))
    _add(
        checks,
        "policy: no wrapper/interpreter command",
        not wrapper_match,
    ...
    _add(
        checks,
        "policy: command is not a local path",
...
def _check_skill(root: Path, checks: list[Check]) -> None:
    ...
    _add(
        checks,
        "projected skill file set present",
    ...
    _add(
        checks,
        "skill name Agent-Skills-conformant",
        name_ok,
    ...
    _add(
        checks,
        f"skill description <= {DESCRIPTION_MAX} chars",
    ...
    _add(
        checks,
        "skill compatibility valid (optional, string, bounded)",
        compatibility is None
        or (
            isinstance(compatibility, str)
            and compatibility.strip()
    ...
    _add(
        checks,
        "skill license valid (optional, string)",
        license_value is None or (isinstance(license_value, str) and bool(license_value.strip())),
        repr(license_value),
    )
    ...
    _add(
        checks,
        "skill allowed-tools valid (optional, string)",
...
def _check_runtime_surface(root: Path, checks: list[Check]) -> None:
    for forbidden_dir in FORBIDDEN_DIRS:
        _add(
            checks, f"no local runtime dir '{forbidden_dir}/'", not (root / forbidden_dir).exists()
        )
    hits: list[str] = []
